  ...
  const executeMoveCall = (method: "name" | "description" | "traits" | "url") => {
    ...
    if (method === "name") {
      txb.moveCall({
        ...
        target: `${minterPackageId}::frens_mint::update_name`,
      });
      ...
      txb.moveCall({
        arguments: [txb.object(id), txb.pure.u64(0)],
        target: `${minterPackageId}::frens_mint::update_traits`,
      });
    } else {
      txb.moveCall({
        arguments: [txb.object(id), txb.pure.u64(0)],
        target: `${minterPackageId}::frens_mint::update_url`,
      });
    }
      ...
      <Flex direction="column" gap="2">
        ...
        <Flex direction="row" gap="2">
          ...
          {ownedByCurrentAccount ? (
            <Button onClick={() => executeMoveCall("name")}>Update Name</Button>
          ) : null}

          {ownedByCurrentAccount ? (
            <Button onClick={() => executeMoveCall("description")}>Update Description</Button>
          ) : null}

          {ownedByCurrentAccount ? (
            <Button onClick={() => executeMoveCall("traits")}>Update Triats</Button>
          ) : null}

          {ownedByCurrentAccount ? (
            <Button onClick={() => executeMoveCall("url")}>Update URL</Button>
          ) : null}
        </Flex>
      </Flex>
    </>
